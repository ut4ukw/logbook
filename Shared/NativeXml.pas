...
type
  ...
  public
    ...
    // procedure will raise an exception of type Exception when it encounters
    ...
    // an exception of type Exception when it encounters non-wellformed XML.
    procedure LoadFromFile(const AFileName: string); virtual;
    // Call procedure ReadFromString to load an XML document from the UTF8String AValue.
    // The ReadFromString procedure will raise an exception of type Exception
...
begin
  ...
  if Count > Length(ADecoded) then
    raise Exception.Create(sxeMissingDataInBinaryStream);
...
begin
  ...
  if BufSize * 4 <> length(BufData) then
    raise Exception.Create(sxeErrorCalcStreamLength);
  BufSize := BufSize * 3;
  ...
  try
    ...
    xeStyleSheet:
      ...
      if LItems.Count = 0 then
        raise Exception.Create(sxeMissingElementName);
            ...
            begin
              ...
              repeat
                ...
                if S.Read(Ch, 1) <> 1 then
                  raise Exception.CreateFmt(sxeMissingCloseTag, [Name]);

                // Is there a subtag?
                if Ch = '<' then
                begin
                  if not Reader.ReadCharSkipBlanks(Ch) then
                    raise Exception.CreateFmt(sxeMissingDataAfterGreaterThan, [Name]);
                  if Ch = '/' then
                  begin

                    // This seems our closing tag
                    if not ReadStringFromStreamUntil(Reader, '>', V, True) then
                      raise Exception.CreateFmt(sxeMissingLessThanInCloseTag, [Name]);
                    if UTF8CompareText(sdUTF8Trim(V), Name) <> 0 then
                      raise Exception.CreateFmt(sxeIncorrectCloseTag, [Name]);
                    V := '';
                    break;
...
begin
  ...
  xeNormal: // normal nodes (xeNormal)
    ...
    raise Exception.Create(sxeIllegalElementType);
  end;//case
  sdUTF8WriteStringToStream(S, Line);
...
begin
  ...
  begin
    ...
    if NormalCount = 0 then
      raise Exception.Create(sxeNoRootElement);
...
begin
  if not assigned(Root) and FParserWarnings then
    raise Exception.Create(sxeRootElementNotDefined);
