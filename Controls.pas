...
(*$HPPEMIT '#pragma alias "@Controls@TWinControl@CreateParentedControl$qqrpv"="@Controls@TWinControl@CreateParentedControl$qqrp6HWND__"' *)
(*$HPPEMIT '#pragma alias "@Controls@TCustomHint@NCPaintHint$qqrp26Controls@TCustomHintWindowpv"="@Controls@TCustomHint@NCPaintHint$qqrp26Controls@TCustomHintWindowp5HDC__"' *)
(*$HPPEMIT '#endif' *)
(*$HPPEMIT '#endif' *)
...
uses
  Messages, Windows, MultiMon, Classes, SysUtils, Graphics, Menus,
  CommCtrl, Imm, ImgList, ActnList, SyncObjs,
{$IF DEFINED(LINUX)}
  WinUtils;
{$IFEND}
{$IF DEFINED(CLR)}
  WinUtils, System.Runtime.InteropServices, System.ComponentModel, System.Security,
...
{$IF DEFINED(CLR)}
  TCustomData = TObject;
  TCustomLongData = TObject;
  TWindowProcPtr = IntPtr;
{$ELSE}
  TCustomData = Pointer;
  TCustomLongData = Longint;
...
{$ELSE}
  TpcShrd,
{$IFEND}
  Consts, Forms, ActiveX, Math, Themes, UxTheme, DwmApi, TypInfo;
...
begin
  ...
  SetWParam(Windows.WPARAM(IntPtr(FWHandle)));
end;
...
begin
  ...
  SetLParam(Windows.LPARAM(IntPtr(FLHandle)));
end;
...
begin
  ...
  begin
    ...
    if FloatHost <> nil then
      Dock(FloatHost, Types.Rect(0, 0, FloatHost.ClientWidth,
...
begin
  Result := Perform(Msg, WParam, Windows.LPARAM(LParam));
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Perform(WM_LBUTTONUP, 0, Windows.LPARAM(Integer($FFFFFFFF)));
    end;
  end
  else
    Exclude(FControlState, csLButtonDown);
...
{$ELSE}
  ...
  Perform(CM_CONTROLLISTCHANGING, Integer(@Item), Integer(False));
{$IFEND}
  Realign;
end;
...
begin
  ...
  try
    if not Windows.DestroyWindow(WindowHandle) then
      RaiseLastOSError;
  finally
...
{$ELSE}
    ...
    EnumChildWindows(FHandle, @DestroyChildWindow, Integer(@LData));
{$IFEND}
    DestroyWnd;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      FParentWindow := Value;
      Windows.SetParent(WindowHandle, Value);
...
{$IFEND}
begin
  P := ScreenToClient(SmallPointToPoint(Types.SmallPoint(GetMessagePos)));
  if Control = nil then
    ...
    begin
      with TControl(FControls[I]) do
        if (Visible and (not (csDesigning in ComponentState) or not (csDesignerHide in ControlState)) or
          ((csDesigning in ComponentState) and not (csDesignerHide in ControlState)) and
          not (csNoDesignVisible in ControlStyle)) and
          RectVisible(DC, Types.Rect(Left, Top, Left + Width,
            Top + Height)) then
        begin
...
{$ELSE}
  if (Perform(CM_DOCKCLIENT, Integer(Source), Integer(SmallPoint(X, Y))) >= 0) and
{$IFEND}
...
var
  I: Integer;
begin
  Result := Types.Rect(MaxInt, MaxInt, 0, 0);
  for I := 0 to ControlCount - 1 do
    with Controls[I] do
      if Visible or (csDesigning in ComponentState) and
...
{$IFEND}
  ...
  begin
    ...
    begin
      ...
      LPoint := Rect.TopLeft;
      Windows.ScreenToClient(ParentHandle, LPoint);
      Rect.TopLeft := LPoint;
...
{$ELSE}
    ChildDockSite.SendDockNotification(CM_INVALIDATEDOCKHOST,
      Integer(Self), Integer(FocusLost));
{$IFEND}
end;
...
procedure TDockTree.LoadFromStream(Stream: TStream);
  ...
  begin
    ...
    begin
      ...
      Stream.Read(LBytes{$IFNDEF CLR}[0]{$ENDIF}, LSize);
  ...
  try
    ...
    begin
      { Read in the relative size "option", and the orientation of
        the top zone }
{$IF DEFINED(CLR)}
      Stream.Read(LByte, SizeOf(Byte));
      FRelativeSizes := LByte <> 0;
      Stream.Read(LByte, SizeOf(Byte));
      TopOrientation := TDockOrientation(LByte);
{$ELSE}
      Stream.Read(FRelativeSizes, SizeOf(FRelativeSizes));
...
procedure TDockTree.PaintDockFrame(Canvas: TCanvas; Control: TControl;
  ...
  var
    ...
    DrawRect: TRect;
  begin
    DrawRect := Types.Rect(Left, Top, Left+FGrabberSize-2,
      Top+FGrabberSize-2);
    if ThemeServices.ThemesEnabled then
    begin
      Details := ThemeServices.GetElementDetails(twCloseButtonNormal);
      ThemeServices.DrawElement(Canvas.Handle, Details, DrawRect, nil);
    end
    else
  ...
  var
    ...
    DrawRect: TRect;
  begin
    DrawRect := Types.Rect(Left, Top, Right, Bottom);
    Details := ThemeServices.GetElementDetails(GripperType);
...
begin
  ...
  begin
    DockRect := Types.Rect(0, 0, FDockSite.ClientWidth,
...
begin
  Windows.SetCursorPos(Value.X, Value.Y);
end;
...
type
  [SuppressUnmanagedCodeSecurity]
  TFNWINNLSEnableIME = function(hwnd: HWnd; bool: LongBool): Boolean; {$IFNDEF CLR}stdcall;{$ENDIF}
  [SuppressUnmanagedCodeSecurity]
  TFNImmGetContext = function(hWnd: HWND): HIMC; {$IFNDEF CLR}stdcall;{$ENDIF}
  [SuppressUnmanagedCodeSecurity]
  TFNImmReleaseContext = function(hWnd: HWND; hImc: HIMC): Boolean; {$IFNDEF CLR}stdcall;{$ENDIF}
  [SuppressUnmanagedCodeSecurity]
  TFNImmGetConversionStatus = function(hImc: HIMC; var Conversion, Sentence: DWORD): Boolean; {$IFNDEF CLR}stdcall;{$ENDIF}
  [SuppressUnmanagedCodeSecurity]
  TFNImmSetConversionStatus = function(hImc: HIMC; Conversion, Sentence: DWORD): Boolean; {$IFNDEF CLR}stdcall;{$ENDIF}
  [SuppressUnmanagedCodeSecurity]
  TFNImmSetOpenStatus = function(hImc: HIMC; fOpen: Boolean): Boolean; {$IFNDEF CLR}stdcall;{$ENDIF}
  [SuppressUnmanagedCodeSecurity]
  TFNImmSetCompositionWindow = function(hImc: HIMC; var lpCompForm: TCOMPOSITIONFORM): Boolean; {$IFNDEF CLR}stdcall;{$ENDIF}
  [SuppressUnmanagedCodeSecurity]
  TFNImmSetCompositionFont = function(hImc: HIMC; var lpLogfont: TLOGFONT): Boolean; {$IFNDEF CLR}stdcall;{$ENDIF}
  [SuppressUnmanagedCodeSecurity]
  TFNImmIsIME = function(hKl: HKL): Boolean; {$IFNDEF CLR}stdcall;{$ENDIF}
  [SuppressUnmanagedCodeSecurity]
  TFNImmNotifyIME = function(hImc: HIMC; dwAction, dwIndex, dwValue: DWORD): Boolean; {$IFNDEF CLR}stdcall;{$ENDIF}
{$IF DEFINED(CLR)}
  [SuppressUnmanagedCodeSecurity]
  TFNImmGetCompositionString = function(hImc: HIMC; dWord1: DWORD; lpBuf: IntPtr; dwBufLen: DWORD): Longint;
{$ELSE}
  TFNImmGetCompositionString = function(hImc: HIMC; dWord1: DWORD; lpBuf: pointer; dwBufLen: DWORD): Longint stdcall;
{$IFEND}

var
  IMM32DLL: THandle = 0;
  _WINNLSEnableIME: TFNWINNLSEnableIME;
  _ImmGetContext: TFNImmGetContext;
  _ImmReleaseContext: TFNImmReleaseContext;
  _ImmGetConversionStatus: TFNImmGetConversionStatus;
  _ImmSetConversionStatus: TFNImmSetConversionStatus;
  _ImmSetOpenStatus: TFNImmSetOpenStatus;
  _ImmSetCompositionWindow: TFNImmSetCompositionWindow;
  _ImmSetCompositionFont: TFNImmSetCompositionFont;
  _ImmGetCompositionString: TFNImmGetCompositionString;
  _ImmIsIME: TFNImmIsIME;
  _ImmNotifyIME: TFNImmNotifyIME;
{$IF DEFINED(CLR)}
  IMM32Handle: TSafeModuleHandle;
{$IFEND}
...
{$IF DEFINED(CLR)}
      BindProcAddress(_WINNLSEnableIME, TypeOf(TFNWINNLSEnableIME), UserHandle, 'WINNLSEnableIME');
{$ELSE}
...
{$IFEND}
    ...
    begin
      ...
      begin
{$IF DEFINED(CLR)}
        IMM32Handle := TSafeModuleHandle.Create(IMM32DLL);
        BindProcAddress(_ImmGetContext, TypeOf(TFNImmGetContext), IMM32DLL, 'ImmGetContext');
        BindProcAddress(_ImmReleaseContext, TypeOf(TFNImmReleaseContext), IMM32DLL, 'ImmReleaseContext');
        BindProcAddress(_ImmGetConversionStatus, TypeOf(TFNImmGetConversionStatus), IMM32DLL, 'ImmGetConversionStatus');
        BindProcAddress(_ImmSetConversionStatus, TypeOf(TFNImmSetConversionStatus), IMM32DLL, 'ImmSetConversionStatus');
        BindProcAddress(_ImmSetOpenStatus, TypeOf(TFNImmSetOpenStatus), IMM32DLL, 'ImmSetOpenStatus');
        BindProcAddress(_ImmSetCompositionWindow, TypeOf(TFNImmSetCompositionWindow), IMM32DLL, 'ImmSetCompositionWindow');
        BindProcAddress(_ImmSetCompositionFont, TypeOf(TFNImmSetCompositionFont), IMM32DLL, 'ImmSetCompositionFontW');
        BindProcAddress(_ImmGetCompositionString, TypeOf(TFNImmGetCompositionString), IMM32DLL, 'ImmGetCompositionStringW');
        BindProcAddress(_ImmIsIME, TypeOf(TFNImmIsIME), IMM32DLL, 'ImmIsIME');
        BindProcAddress(_ImmNotifyIME, TypeOf(TFNImmNotifyIME), IMM32DLL, 'ImmNotifyIME');
{$ELSE}
        ...
        @_ImmGetCompositionString := GetProcAddress(IMM32DLL, 'ImmGetCompositionStringW');
...
begin
  Result := Imm32SetCompositionWindow(hImc, lpCompForm{$IFNDEF CLR}^{$ENDIF})
end;
{$IFEND}
...
begin
  Result := Imm32SetCompositionFont(hImc, lpLogFont{$IFNDEF CLR}^{$ENDIF})
end;
{$IFEND}
